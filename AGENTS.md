...
other apex and is in the `test-api` matrix. All twenty units implement the
...
- Every unit runs the engine through `@tailwindcss/vite`, because every unit
  builds with Vite. There is no `postcss.config.mjs` anywhere and
  `@tailwindcss/postcss` is not installed. Vite emits the stylesheet into `dist/client`
...
`adr/013-frames-tanstack-start.md` records the trade-off. Regenerating produces no git diff, which is the point
