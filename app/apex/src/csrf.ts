 ...
 * route appears.
 */
const PRODUCTION_APEX_ORIGIN = /^https:\/\/umaxica\.app$/u;

/*
 * The local development origin, and only on a non-production deployment.
 *
 * `vite dev` and `wrangler dev` are reached at `http://app.localhost:PORT`,
 ...
 */
const LOCAL_APEX_ORIGIN = /^http:\/\/app\.localhost(?::\d+)?$/u;
 ...
 *
 * The pattern this replaced was `<label>.app-apex.workers.dev`, a hostname
 ...
 * `umaxica-apps-edge-app-apex`, so it matched no real preview at all.
 */
const PREVIEW_APEX_ORIGIN =
  /^https:\/\/(?:[a-z0-9-]+-)?umaxica-apps-edge-app-apex\.[a-z0-9-]+\.workers\.dev$/u;
