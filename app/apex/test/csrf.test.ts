...
const PRODUCTION_ORIGIN = 'https://umaxica.app';
const LOCAL_ORIGIN = 'http://app.localhost:5401';

describe('apex CSRF config', () => {
  it("accepts this unit's own apex origin and nothing adjacent to it", () => {
    expect(isAllowedApexOrigin('https://umaxica.app', production)).toBe(true);
    expect(isAllowedApexOrigin('http://umaxica.app', production)).toBe(false);
    expect(isAllowedApexOrigin('https://umaxica.app.evil.example', production)).toBe(false);
  ...
  it('accepts the local dev origin only off production', () => {
    expect(isAllowedApexOrigin('http://app.localhost:3333', development)).toBe(true);
    expect(isAllowedApexOrigin('http://app.localhost', development)).toBe(true);
    expect(isAllowedApexOrigin('http://app.localhost:3333', production)).toBe(false);
  ...
  it('accepts previews of this worker on workers.dev', () => {
    expect(
      isAllowedApexOrigin('https://umaxica-apps-edge-app-apex.acct.workers.dev', production),
    ).toBe(true);
    expect(
      isAllowedApexOrigin('https://abc123-umaxica-apps-edge-app-apex.acct.workers.dev', production),
    ).toBe(true);
    expect(
      isAllowedApexOrigin('http://umaxica-apps-edge-app-apex.acct.workers.dev', production),
    ).toBe(false);
    expect(
...
describe('apex CSRF environment gate', () => {
  const postFrom = (origin: string, env?: { EDGE_ENV?: string }) => {
    const request = new Request('http://app.localhost/about', { method: 'POST' });
    /*
    ...
    return createApexApp(() => undefined, { service: 'app' }).request(request, undefined, env);
  };
