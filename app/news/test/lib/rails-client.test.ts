...
describe('app/news rails client', () => {
  afterEach(() => {
  ...
  it('uses the VPC binding when present', async () => {
    ...
    expect(new URL(requestUrl).host).toBe('news.app.localhost:3000');
  ...
  it('uses the private Podman transport only for explicit local Node development', async () => {
    ...
    expect(new URL(requestUrl).origin).toBe('http://news.app.localhost:3000');
