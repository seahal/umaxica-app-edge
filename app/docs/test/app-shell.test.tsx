...
describe('app/docs application shell', () => {
  it('renders the public page inside the shell', async () => {
    const html = await renderDocument('/');

    expect(html).toContain('UMAXICA ドキュメント');
  ...
  it("renders the page on its own too, so the copy is the page's and not the shell's", () => {
    ...
    expect(renderToStaticMarkup(<Home />)).toContain('使い方と技術情報をまとめています');
  });
  ...
  it('gives the index route the document default title', () => {
    expect(headTitleOf(indexRoute)).toBe('Docs — UMAXICA (APP)');
  });
