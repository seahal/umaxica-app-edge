...
describe('normalizeProxyErrorCode', () => {
  ...
  it.each(['something_new', 'core.app.localhost', ''])('folds %s into unknown', (code) => {
    ...
    [
      ...
      () =>
        vi
          .fn()
          .mockRejectedValue(
            new Error('connect ECONNREFUSED core.app.localhost:3000 SECRET_API_KEY'),
          ),
    ],
    [
      'a ProxyError 500',
      () =>
        vi.fn().mockResolvedValue(
          new Response('ProxyError: connection_refused core.app.localhost', {
            status: 500,
            headers: { 'content-type': 'text/plain' },
          }),
        ),
