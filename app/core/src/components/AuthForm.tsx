...
import { Button, Checkbox, Form, Input, Label, TextField } from "react-aria-components";
...
export function AuthForm({ type, onSubmit }: AuthFormProps) {
  ...
  return (
    <Form onSubmit={handleSubmit} className="space-y-4">
      {/* 新規登録の場合は名前入力 */}
      {!isLogin && (
        <TextField value={name} onChange={setName} isRequired className="space-y-2">
      ...
      {/* パスワード入力 */}
      <TextField value={password} onChange={setPassword} isRequired className="space-y-2">
        <Label className="block text-sm font-semibold text-gray-700 dark:text-gray-300">
          パスワード
        </Label>
      ...
      {/* 新規登録の場合：利用規約への同意 */}
      {!isLogin && (
        <Checkbox isSelected={agreed} onChange={setAgreed} className="flex items-start gap-2 group">
          ...
          <span className="text-sm text-gray-600 dark:text-gray-400">
            <a href="/legal/terms" className="text-blue-500 hover:underline dark:text-blue-400">
              利用規約
            </a>
            と
            <a href="/legal/privacy" className="text-blue-500 hover:underline dark:text-blue-400">
              プライバシーポリシー
            </a>
            に同意します
          </span>
...
export function SocialLoginButton({ provider, onClick }: SocialLoginButtonProps) {
  const configs = {
    ...
    twitter: {
      ...
      bgColor: "bg-black hover:bg-gray-900 dark:bg-gray-900 dark:hover:bg-black",
