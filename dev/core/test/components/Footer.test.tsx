...
describe('Footer component', () => {
  ...
  it('renders quick links with expected routes', () => {
    ...
    expect(links).toContainEqual({ href: '/', name: 'ホーム' });
    expect(links).toContainEqual({ href: '/about', name: 'About' });
    expect(links).toContainEqual({ href: '/contact', name: 'お問い合わせ' });
  });
  ...
  it('includes privacy, terms and status legal links', () => {
    const document = renderFooter();
    ...
    const status = links.find((link) => link.textContent?.trim() === 'ステータス');
