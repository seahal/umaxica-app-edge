...
describe('buildBrandTitle', () => {
  it('formats as "Page — UMAXICA (TLD)" when pageTitle is provided', () => {
    const result = buildBrandTitle('Pricing', { brandName: 'UMAXICA', tld: 'DEV' });
    expect(result).toBe('Pricing — UMAXICA (DEV)');
  });
  ...
  it('uses defaultPageTitle when pageTitle is not provided', () => {
    ...
    expect(result).toBe('Home — UMAXICA (DEV)');
  });

  it('returns the root title when pageTitle and defaultPageTitle are both missing', () => {
    const result = buildBrandTitle(undefined, { brandName: 'UMAXICA', tld: 'DEV' });
    expect(result).toBe('UMAXICA (DEV)');
  });
  ...
  it('never leaks a surface or runtime name into the title', () => {
    const result = buildBrandTitle('About', { brandName: 'UMAXICA', tld: 'DEV' });
