 ...
 * route appears.
 */
const PRODUCTION_APEX_ORIGIN = /^https:\/\/umaxica\.dev$/u;

/*
 * The local development origin, and only on a non-production deployment.
 *
 * `vite dev` and `wrangler dev` are reached at `http://dev.localhost:PORT`,
 ...
 */
const LOCAL_APEX_ORIGIN = /^http:\/\/dev\.localhost(?::\d+)?$/u;
 ...
 *
 * The pattern this replaced was `<label>.dev-apex.workers.dev`, a hostname
 ...
 * `umaxica-apps-edge-dev-apex`, so it matched no real preview at all.
 */
const PREVIEW_APEX_ORIGIN =
  /^https:\/\/(?:[a-z0-9-]+-)?umaxica-apps-edge-dev-apex\.[a-z0-9-]+\.workers\.dev$/u;
