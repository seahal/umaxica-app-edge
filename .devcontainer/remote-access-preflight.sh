...
state_volume=umaxica-apps-edge_tailscale-state
doc=docs/development/tailscale-remote-ssh.md
...
if podman volume exists "${state_volume}" 2> /dev/null; then
  ...
  if [[ -n ${authkey} ]]; then
    fail "TS_AUTHKEY is still set, but umaxica-edge-core is already enrolled." \
