#!/usr/bin/env bash
# `core`'s main process when `compose.custom.yaml` is loaded.
#
...
#
# Runs as `edge`. Prepares the persistent host key, refuses to start without
...
state_dir=/home/edge/.local/state/remote-sshd
host_key=${state_dir}/ssh_host_ed25519_key
authorized_keys=/home/edge/.config/umaxica/authorized_keys
...
if [[ ! -s ${authorized_keys} ]]; then
  ...
  echo 'compose.custom.yaml binds it read-only from .secrets/codex_authorized_keys' >&2
...
if [[ ! -f ${host_key} ]]; then
  ...
  ssh-keygen -q -t ed25519 -N '' -C 'umaxica-apps-edge core' -f "${host_key}"
fi
chmod 0600 "${host_key}"
  ...
  umask 077
{
  ...
  printf 'export PATH=%q\n' "/home/edge/.local/bin:/home/edge/.local/share/pnpm/bin:${PATH}"
} > "${session_env}"
)
...
# tailnet in userspace-networking mode, which runs as `edge` with no
...
ts_state=/home/edge/.local/state/tailscale
