...
6. **Revoke the key, then delete the line.**
   ...
   starts from `umaxica-apps-edge_tailscale-state` alone.
...
(`podman logs umaxica-apps-edge_core_1`), so `known_hosts` can be verified rather than
...
```bash
ssh umaxica-edge-core
hostname; pwd; git status --short; node --version; pnpm --version
```

`hostname` reports the `core` container, and `pwd` is `/home/edge/workspace` — the
...
- **The sshd is unprivileged.** `core` runs `cap_drop: ALL` with
  ...
  cannot use PAM, and cannot privilege-separate. Port 2222 and `AllowUsers edge`
  follow from that.

- **This image is Node and pnpm.** `ruby`, `bundle` and `bun` are not present and
  are not expected to be: `AGENTS.md` forbids bun outright, and Ruby lives in
  `umaxica-apps-global`. A Codex task that needs Rails belongs in that repository's
  container, reachable at `umaxica-global-core` over the same tailnet.
