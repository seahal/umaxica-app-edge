...
describe('buildBrandTitle', () => {
  it('formats as "Page — UMAXICA (TLD)" when pageTitle is provided', () => {
    const result = buildBrandTitle('Pricing', { brandName: 'UMAXICA', tld: 'COM' });
    expect(result).toBe('Pricing — UMAXICA (COM)');
  });
  ...
  it('uses defaultPageTitle when pageTitle is not provided', () => {
    ...
    expect(result).toBe('Home — UMAXICA (COM)');
  });

  it('returns the root title when pageTitle and defaultPageTitle are both missing', () => {
    const result = buildBrandTitle(undefined, { brandName: 'UMAXICA', tld: 'COM' });
    expect(result).toBe('UMAXICA (COM)');
  });
