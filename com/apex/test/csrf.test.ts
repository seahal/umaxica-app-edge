...
const PRODUCTION_ORIGIN = 'https://umaxica.com';
const LOCAL_ORIGIN = 'http://com.localhost:5101';

describe('apex CSRF config', () => {
  it("accepts this unit's own apex origin and nothing adjacent to it", () => {
    expect(isAllowedApexOrigin('https://umaxica.com', production)).toBe(true);
    expect(isAllowedApexOrigin('http://umaxica.com', production)).toBe(false);
    expect(isAllowedApexOrigin('https://umaxica.com.evil.example', production)).toBe(false);
  ...
  it('rejects the sibling apex domains', () => {
    ...
    expect(isAllowedApexOrigin('https://umaxica.app', production)).toBe(false);
  });

  it('accepts the local dev origin only off production', () => {
    expect(isAllowedApexOrigin('http://com.localhost:3333', development)).toBe(true);
    expect(isAllowedApexOrigin('http://com.localhost', development)).toBe(true);
    expect(isAllowedApexOrigin('http://com.localhost:3333', production)).toBe(false);
    expect(isAllowedApexOrigin('http://org.localhost:3333', development)).toBe(false);
  });
  ...
  it('accepts previews of this worker on workers.dev', () => {
    ...
    expect(
      isAllowedApexOrigin('https://abc123-umaxica-apps-edge-com-apex.acct.workers.dev', production),
    ).toBe(true);
    expect(
      isAllowedApexOrigin('http://umaxica-apps-edge-com-apex.acct.workers.dev', production),
    ).toBe(false);
    expect(
      isAllowedApexOrigin('https://umaxica-apps-edge-org-apex.acct.workers.dev', production),
    ).toBe(false);
