...
describe('com/news rails client factory', () => {
  it('always requests against the fixed hostname regardless of caller input', async () => {
    ...
    const client = createRailsClient(binding, 'http://news.com.localhost:3000');

    await client.fetch('/edge/v0/health');

    const [requestUrl] = binding.fetch.mock.calls[0] as [string, RequestInit];
    expect(new URL(requestUrl).hostname).toBe('news.com.localhost');
  ...
  it('combines a relative path with the fixed origin correctly', async () => {
    ...
    const [requestUrl] = binding.fetch.mock.calls[0] as [string, RequestInit];
    expect(requestUrl).toBe('http://news.com.localhost:3000/edge/v0/widgets?limit=10');
  });
  ...
  it('sends the path through unchanged, with no frame prefix', async () => {
    ...
    const [requestUrl] = binding.fetch.mock.calls[0] as [string, RequestInit];
    expect(requestUrl).toBe('http://news.com.localhost:3000/health/liveness.json');
  });
});
