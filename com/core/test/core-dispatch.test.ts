...
describe('com/core classifyCorePath', () => {
  it.each([
...
describe('com/core blockedCoreResponse', () => {
...
describe('com/core dispatchToRails', () => {
  it('returns 503 when no VPC binding is present', async () => {
    const response = await dispatchToRails(new Request('https://jp.umaxica.com/api/v0/x'), {});
    expect(response.status).toBe(503);
  });

  it('builds the Rails request against the public hostname, not a VPC routing label', async () => {
    const fetch = vi.fn().mockResolvedValue(new Response('ok'));
    await dispatchToRails(new Request('https://jp.umaxica.com/api/v0/x'), {
      UMAXICA_APPS_EDGE_CF_WORKERS_VPC: { fetch } as unknown as Fetcher,
    });
    const request = fetch.mock.calls[0]?.[0] as Request;
    expect(new URL(request.url).host).toBe('jp.umaxica.com');
  });
  ...
  it('strips caller-controlled forwarding headers before dispatch', async () => {
    ...
    await dispatchToRails(
      new Request('https://jp.umaxica.com/api/v0/x', {
