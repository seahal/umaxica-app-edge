  ...
  test: {
    coverage: {
      exclude: [
        ...
        '**/next.config.ts',
        '**/open-next.config.ts',
        // Build glue: imports `../../.open-next/worker.js`, which only exists
        // after `opennextjs-cloudflare build`. Every test vi.mocks this module,
        // so it is never really executed; including it also makes the coverage
        // remapper fail to parse its `as` assertion and drop the per-file table.
        '**/src/lib/next-handler.ts',
        '**/vitest.config.ts',
        '**/vitest.setup.ts',
      ],
