...
describe('com/info rails client', () => {
  afterEach(() => {
  ...
  it('uses the VPC binding when present', async () => {
    ...
    expect(new URL(requestUrl).host).toBe('info.com.localhost:3000');
  ...
  it('uses the private Podman transport only for explicit local Node development', async () => {
    ...
    expect(new URL(requestUrl).origin).toBe('http://info.com.localhost:3000');
