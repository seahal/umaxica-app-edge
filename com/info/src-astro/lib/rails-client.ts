 ...
 *
 * 1. No `import '@tanstack/react-start/server-only'` — Astro decides server vs
 *    client by file location (`src-astro/pages/*.ts` endpoints are server-only)
 ...
 *    moves to the call site (`src-astro/pages/health.ts`).
 *
...
const PRIVATE_RAILS_ORIGIN = 'http://info.com.localhost:3000';
