...
const PRODUCTION_ORIGIN = 'https://umaxica.net';
const LOCAL_ORIGIN = 'http://net.localhost:5201';

describe('apex CSRF config', () => {
  it("accepts this unit's own apex origin and nothing adjacent to it", () => {
    expect(isAllowedApexOrigin('https://umaxica.net', production)).toBe(true);
    expect(isAllowedApexOrigin('http://umaxica.net', production)).toBe(false);
    expect(isAllowedApexOrigin('https://umaxica.net.evil.example', production)).toBe(false);
  ...
  it('accepts the local dev origin only off production', () => {
    expect(isAllowedApexOrigin('http://net.localhost:3333', development)).toBe(true);
    expect(isAllowedApexOrigin('http://net.localhost', development)).toBe(true);
    expect(isAllowedApexOrigin('http://net.localhost:3333', production)).toBe(false);
  ...
  it('accepts previews of this worker on workers.dev', () => {
    expect(
      isAllowedApexOrigin('https://umaxica-apps-edge-net-apex.acct.workers.dev', production),
    ).toBe(true);
    expect(
      isAllowedApexOrigin('https://abc123-umaxica-apps-edge-net-apex.acct.workers.dev', production),
    ).toBe(true);
    expect(
      isAllowedApexOrigin('http://umaxica-apps-edge-net-apex.acct.workers.dev', production),
    ).toBe(false);
    expect(
...
describe('apex CSRF environment gate', () => {
  const postFrom = (origin: string, env?: { EDGE_ENV?: string }) => {
    const request = new Request('http://net.localhost/about', { method: 'POST' });
    /*
