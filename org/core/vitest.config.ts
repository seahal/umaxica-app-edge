  ...
  test: {
    coverage: {
      exclude: [
        ...
        // the Worker build and nowhere else, so no Node test can import it. What
        // it wires — the dispatch boundary in `src/worker.ts` and the security
        // headers — is covered by `test/worker.test.ts` and
        // `test/content-security-policy.test.ts`. `src/router.tsx` is
        // deliberately NOT excluded: the tests drive a real router through it.
