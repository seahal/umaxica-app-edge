 ...
 * route appears.
 */
const PRODUCTION_APEX_ORIGIN = /^https:\/\/umaxica\.org$/u;

/*
 * The local development origin, and only on a non-production deployment.
 *
 * `vite dev` and `wrangler dev` are reached at `http://org.localhost:PORT`,
 ...
 */
const LOCAL_APEX_ORIGIN = /^http:\/\/org\.localhost(?::\d+)?$/u;
 ...
 *
 * The pattern this replaced was `<label>.org-apex.workers.dev`, a hostname
 ...
 * `umaxica-apps-edge-org-apex`, so it matched no real preview at all.
 */
const PREVIEW_APEX_ORIGIN =
  /^https:\/\/(?:[a-z0-9-]+-)?umaxica-apps-edge-org-apex\.[a-z0-9-]+\.workers\.dev$/u;
