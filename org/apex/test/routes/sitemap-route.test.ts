...
describe('GET /sitemap.xml', () => {
  it('returns XML with correct content type', async () => {
    const response = await requestFromOrgApp('/sitemap.xml');
  ...
  it('includes umaxica.org URLs', async () => {
    const response = await requestFromOrgApp('/sitemap.xml');
    const body = await response.text();

    expect(body).toContain('<loc>https://umaxica.org/</loc>');
    expect(body).toContain('<loc>https://umaxica.org/about</loc>');
    expect(body).toContain('<loc>https://umaxica.org/health</loc>');
  });
