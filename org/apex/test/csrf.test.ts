...
const PRODUCTION_ORIGIN = 'https://umaxica.org';
const LOCAL_ORIGIN = 'http://org.localhost:5301';

describe('apex CSRF config', () => {
  it("accepts this unit's own apex origin and nothing adjacent to it", () => {
    expect(isAllowedApexOrigin('https://umaxica.org', production)).toBe(true);
    expect(isAllowedApexOrigin('http://umaxica.org', production)).toBe(false);
    expect(isAllowedApexOrigin('https://umaxica.org.evil.example', production)).toBe(false);
  ...
  it('accepts the local dev origin only off production', () => {
    expect(isAllowedApexOrigin('http://org.localhost:3333', development)).toBe(true);
    expect(isAllowedApexOrigin('http://org.localhost', development)).toBe(true);
    expect(isAllowedApexOrigin('http://org.localhost:3333', production)).toBe(false);
  ...
  it('accepts previews of this worker on workers.dev', () => {
    ...
    expect(
      isAllowedApexOrigin('https://abc123-umaxica-apps-edge-org-apex.acct.workers.dev', production),
    ).toBe(true);
    expect(
      isAllowedApexOrigin('http://umaxica-apps-edge-org-apex.acct.workers.dev', production),
    ).toBe(false);
    expect(
...
describe('apex CSRF environment gate', () => {
  const postFrom = (origin: string, env?: { EDGE_ENV?: string }) => {
    ...
    return createApexApp(() => undefined, { service: 'org' }).request(request, undefined, env);
  };
