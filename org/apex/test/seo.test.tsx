...
describe('seo helpers', () => {
  ...
  it('SeoHead outputs SEO tags from route metadata', async () => {
    ...
    app.get('/page', (c) => {
      ...
      const html = renderToString(<SeoHead c={c} brand={{ brandName: 'UMAXICA', tld: 'ORG' }} />);
      return c.html(html);
    });
    ...
    expect(body).toContain('<title>Pricing — UMAXICA (ORG)</title>');
    ...
    expect(body).toContain('<meta property="og:title" content="Pricing — UMAXICA (ORG)"/>');
  ...
  it('title is the bare root title when no pageTitle/defaultPageTitle are present', async () => {
    ...
    const body = await res.text();
    expect(body).toContain('<title>UMAXICA (ORG)</title>');
  });
  ...
  it('uses default metadata and omits blank optional tags', async () => {
    ...
    app.get('/default-meta', (c) => {
      const html = renderToString(
        <SeoHead
          c={c}
          brand={{ brandName: 'UMAXICA', tld: 'ORG' }}
          defaultMeta={{
            title: '   ',
            pageTitle: 'Home',
            description: '   ',
    ...
    expect(body).toContain('<title>Home — UMAXICA (ORG)</title>');
    expect(body).toContain('<meta name="twitter:card" content="summary_large_image"/>');
